<template>
    <div class="product-viewer">
        ...
        <van-image v-else class="img" :src="backup_img" :class="{ loop: notSupport }"
...
const canvasWidth = ref((<number>px2rem(380, true)) * (parseFloat(document.getElementsByTagName('html')[0].style.fontSize.split('px')[0])))
...
onMountedOrActivated(async () => {
    if (webglSupport.value) {
        ...
        function animate() {
            ...
            if (productModel) {
                if (rotate_mode.value === RotateMode.FULL) {
                    ...
                    period_delta += 0.1;
                    productModel.rotation.y += 0.005 * Math.sin(0.5 * period_delta)
                }
            }
        ...
        document.querySelector('#product-model-viewer')!.appendChild(renderer.domElement)
        param = {
...
const onWindowResize = () => {
    canvasWidth.value = (<number>px2rem(380, true)) * (parseFloat(document.getElementsByTagName('html')[0].style.fontSize.split('px')[0]))
    if (camera && renderer) {
